...
function DB(credentials) {
  const DB_NAME = credentials.DB_NAME || 'todos';
  const COLLECTION_NAME = credentials.COLLECTION_NAME || 'ynov';
  const self = this;
  let db;
  ...
  self.init = () => {
    return new Promise((resolve, reject) => {
      ...
      const ca = [Buffer.from(credentials.MONGO_CERTIFICATE_BASE64, 'base64')];

      // We always want to make a validated TLS/SSL connection
      const options = {
        ssl: credentials.SSL || true,
        sslValidate: credentials.SSL_VALIDATE || true,
        sslCA: MONGO_CERTIFICATE_BASE64 ? ca : null,
        poolSize: 1,
